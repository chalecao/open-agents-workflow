...
test("onboarding v2 — welcome → source → role → use_case (skip path)", async ({ page }) => {
  ...
  // 2. Source step
  await expect(page.getByText("How did you hear about MultiAgent?")).toBeVisible({ timeout: 10000 });
  ...
  // 4. Use case step
  await expect(page.getByText("What do you want to use MultiAgent for?")).toBeVisible({ timeout: 10000 });
...
test("onboarding v2 — zh-Hans renders Chinese labels", async ({ page, context }) => {
  ...
  await expect(page.getByText("你是从哪里了解到 MultiAgent 的？")).toBeVisible({ timeout: 10000 });
  await page.waitForTimeout(500);
