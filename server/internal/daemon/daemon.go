...
type Daemon struct {
	...
	// taskRepoHints maps a running task's ID to the URL→default_branch_hint
	// table for any github_repo resources attached to the issue's project.
	// The repo/checkout health handler consults this map so the daemon can
	// pin a new worktree to the project-configured base branch without the
	// agent having to know — the agent just calls `multica repo checkout
	// <url>` and the daemon applies the hint. Hints are inserted at the
	// start of handleTask and removed on return, so a stale entry can't
	// outlive the task that registered it.
	taskRepoHintsMu sync.RWMutex
	taskRepoHints   map[string]map[string]string // task_id -> url -> hint
...
// registerTaskRepoHints records the URL→default_branch_hint table from the
// task's github_repo project resources so the /repo/checkout health handler
// can apply a project-configured base ref even when the agent calls
// `multica repo checkout <url>` with no `--ref`. Called from handleTask
// before the agent process spawns; the matching unregister is deferred
// onto the same call so a crashed / cancelled task can't leave a stale
// entry behind for an unrelated future task to inherit.
func (d *Daemon) registerTaskRepoHints(taskID string, resources []ProjectResourceData) {
	hints := extractRepoBranchHints(resources)
	if len(hints) == 0 {
		return
	}
	d.taskRepoHintsMu.Lock()
	defer d.taskRepoHintsMu.Unlock()
	d.taskRepoHints[taskID] = hints
}

// unregisterTaskRepoHints removes the task's hint table. Safe to call on a
// task ID that was never registered.
func (d *Daemon) unregisterTaskRepoHints(taskID string) {
	d.taskRepoHintsMu.Lock()
	defer d.taskRepoHintsMu.Unlock()
	delete(d.taskRepoHints, taskID)
}

// lookupTaskRepoHint returns the configured default_branch_hint for the
// given URL within the task's project, or "" when the task has no matching
// hint (or the task isn't tracked here). The empty-string return is what
// `CreateWorktree` already uses to mean "fall back to the remote default
// branch" — see resolveBaseRef in repocache/cache.go — so callers can
// pass the result straight through to params.Ref.
func (d *Daemon) lookupTaskRepoHint(taskID, url string) string {
	if taskID == "" || url == "" {
		return ""
	}
	d.taskRepoHintsMu.RLock()
	defer d.taskRepoHintsMu.RUnlock()
	hints, ok := d.taskRepoHints[taskID]
	if !ok {
		return ""
	}
	return hints[url]
...
// extractRepoBranchHints pulls the URL→default_branch_hint mapping out of
// the github_repo resources attached to a project. Resources of other
// types are ignored, and a github_repo without a hint contributes an empty
// string so a later update can re-base onto the remote default explicitly.
func extractRepoBranchHints(resources []ProjectResourceData) map[string]string {
	if len(resources) == 0 {
		return nil
	}
	...
	for _, r := range resources {
		...
		out[url] = strings.TrimSpace(ref.DefaultBranchHint)
	}
	if len(out) == 0 {
		return nil
	}
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by MultiAgent Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
func (d *Daemon) handleTask(ctx context.Context, task Task, slot int) {
	...
	// Register the task's github_repo default_branch_hint table before any
	// agent process can spawn, so an early `multica repo checkout <url>` call
	// from inside the agent sees the configured base ref. Deferred unregister
	// keeps the map strictly scoped to the task's lifetime — a panic, a
	// server-side cancel, or a normal completion all release the entry.
	d.registerTaskRepoHints(task.ID, task.ProjectResources)
	defer d.unregisterTaskRepoHints(task.ID)
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// up stale MultiAgent instructions (issue id, trigger comment id, reply
	...
	// can call the MultiAgent API and the local daemon (e.g. `multica repo checkout`).
	...
	// MultiAgent API. Prefer the task-scoped token the server minted at claim
