...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, antigravity, qodercli
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	// Qoder CLI mirrors antigravity's model-selection gap: no `--model`
	// flag, model lives in the user's local Qoder settings. Probe without
	// seeding a model env var for the same reason as antigravity.
	if e, ok := probe("MULTICA_QODERCLI_PATH", "qodercli", ""); ok {
		agents["qodercli"] = e
	}
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor-agent, kimi, kiro-cli, agy, or qodercli and ensure it is on PATH")
	}
