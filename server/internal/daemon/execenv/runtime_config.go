...
func runtimeConfigPath(workDir, provider string) string {
	...
	case "codex", "copilot", "opencode", "openclaw", "hermes", "pi", "cursor", "kimi", "kiro", "antigravity", "qodercli":
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ProjectID != "" || len(ctx.ProjectResources) > 0 {
		...
		if len(ctx.ProjectResources) > 0 {
			...
			b.WriteString("If the resource has a `default_branch` configured (shown in this section as `default branch: \\`<name>\\``), the daemon will automatically base the new worktree on it — you do NOT need to pass `--ref` for that case. ")
			b.WriteString("Pass `--ref <branch-or-sha>` only when a task or handoff names an exact different revision (review/QA on a specific commit, hotfix off an older tag, etc.).\n\n")
		} else {
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "copilot", "opencode", "openclaw", "pi", "cursor", "kimi", "kiro", "antigravity", "qodercli":
			// Codex, Copilot, OpenCode, OpenClaw, Pi, Cursor, Kimi, Kiro,
			// Antigravity, and Qoder CLI discover skills natively from
			// their respective paths. For OpenClaw, the daemon also writes
			// a per-task openclaw-config.json (exported via OPENCLAW_CONFIG_PATH)
			// that pins agents.defaults.workspace to the task workdir so
			// the CLI's scanner picks up {workDir}/skills/. Antigravity
			// inherits Gemini CLI's workspace skill layout —
