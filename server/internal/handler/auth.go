...
type UserResponse struct {
	...
	// IsPlatformAdmin mirrors the operator-configured MULTICA_ADMIN_EMAILS
	// allowlist. When true, the caller may create new workspaces and
	// perform every write against every workspace (update / delete /
	// member management / integration control) regardless of the
	// workspace-level role they hold. False on every self-host
	// deployment where the env var is unset, and on every user whose
	// email is not on the list. The UI uses it to hide the matching
	// "Create workspace" / "Settings" affordances for non-admins. See
	// isPlatformAdmin on the Handler for the authoritative check; this
	// flag is a UI mirror, the server never trusts it.
...
func userToResponse(u db.User, isPlatformAdmin bool) UserResponse {
...
func (h *Handler) VerifyCode(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, LoginResponse{
		Token: tokenString,
		User:  userToResponse(user, h.isPlatformAdmin(user.Email)),
	})
}
...
func (h *Handler) GetMe(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, userToResponse(user, h.isPlatformAdmin(user.Email)))
}
...
func (h *Handler) UpdateMe(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, userToResponse(updatedUser, h.isPlatformAdmin(updatedUser.Email)))
}
