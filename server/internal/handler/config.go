...
type AppConfig struct {
	...
	// AdminEmailsConfigured mirrors the server-side MULTICA_ADMIN_EMAILS
	// env var (true when the allowlist is non-empty). The per-user
	// `is_platform_admin` flag on /api/me is the only authoritative
	// source for *which* users can create / manage workspaces; this
	// config-level flag is just the UI's signal that the gate exists
	// at all — without it the frontend can't tell "not on the
	// allowlist" from "no allowlist configured" and would either
	// always hide "Create workspace" (breaking every deployment that
	// hasn't opted in) or never hide it (defeating the feature).
	// Omitted from the JSON when false for the same backwards-compat
	// reason as WorkspaceCreationDisabled.
...
func (h *Handler) GetConfig(w http.ResponseWriter, r *http.Request) {
	config := AppConfig{
		...
		AdminEmailsConfigured:     h.platformAdminGateConfigured(),
	}
	if h.Storage != nil {
