...
type IssueResponse struct {
	...
	HandoffData map[string]any          `json:"handoff_data"`
...
func issueToResponse(i db.Issue, issuePrefix string) IssueResponse {
	...
	return IssueResponse{
		...
		HandoffData:   parseIssueHandoffData(i.HandoffData),
	}
}
...
type CreateIssueRequest struct {
	...
	// HandoffData is a free-form JSON object the autopilot handoff engine
	// reads. Optional on create (the column default is '{}'::jsonb);
	// clients who want to seed the blob up front can pass any object —
	// validation enforces object shape + 8 KiB cap (see
	// validateIssueHandoffData in issue_metadata.go).
	HandoffData json.RawMessage `json:"handoff_data,omitempty"`
...
func (h *Handler) CreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	// Optional handoff_data seed. The autopilot handoff engine reads
	// this blob to gate `issue_has_handoff_data` rules and to render
	// the {{handoff_data}} template token. We validate up front so a
	// 4xx surfaces before the duplicate-guard dance / counter
	// increment run. Empty input is treated as "leave default" — the
	// column DEFAULT '{}'::jsonb kicks in. Sending a non-empty object
	// overrides the default and seeds the blob atomically with the
	// INSERT.
	var handoffDataBytes []byte
	if len(req.HandoffData) > 0 {
		if err := validateIssueHandoffData(req.HandoffData); err != nil {
			writeError(w, http.StatusBadRequest, err.Error())
			return
		}
		handoffDataBytes = []byte(req.HandoffData)
	}
...
type UpdateIssueRequest struct {
	...
	// HandoffData is a tri-state PATCH: omitted / null leaves the column
	// alone, an explicit {} clears it. The handler hands a present
	// value to SetIssueHandoffData, which re-validates the same
	// constraints CreateIssue applies (object shape + 8 KiB cap).
	HandoffData json.RawMessage `json:"handoff_data,omitempty"`
}
...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	// handoff_data is a separate column with its own SQL setter
	// (SetIssueHandoffData) — UpdateIssue's COALESCE-only UPDATE doesn't
	// touch it, so a tri-state PATCH on handoff_data needs an explicit
	// round-trip here. Gated on raw-presence so a PATCH that omits the
	// key leaves the column alone.
	if _, ok := rawFields["handoff_data"]; ok {
		if err := validateIssueHandoffData(req.HandoffData); err != nil {
			writeError(w, http.StatusBadRequest, err.Error())
			return
		}
		...
		if err != nil {
			...
			slog.Warn("SetIssueHandoffData failed", append(logger.RequestAttrs(r), "error", err, "issue_id", id, "workspace_id", workspaceID)...)
			writeError(w, http.StatusInternalServerError, "failed to update handoff_data")
			return
		}
		issue = updated
	}
