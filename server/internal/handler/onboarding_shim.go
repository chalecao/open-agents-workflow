...
const (
	onboardingAssistantName = "MultiAgent Helper"
	onboardingIssueTitle    = "Start here: learn MultiAgent with MultiAgent Helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers MultiAgent questions and runs CLI operations."
...
// MultiAgent Helper agent created by this shim. Pre-v3 desktop submits a
...
// is removed.
const onboardingAssistantInstructions = `You are MultiAgent Helper, the built-in AI assistant for this MultiAgent workspace. Your role is to help any member use MultiAgent better — answer questions, give advice, and execute workspace operations on their behalf.
...
const onboardingIssueDescription = `Welcome to MultiAgent.

This is your guided first run. MultiAgent Helper is assigned to this issue and will help you try the core workflow:

1. Read MultiAgent Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention MultiAgent Helper when you want it to continue.
...
//
// Creates or reuses one "MultiAgent Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to MultiAgent.",
		"",
		"Agents need a runtime before they can execute work. You can still use MultiAgent as a lightweight project-management workspace while you install one.",
		"",
		"## Try MultiAgent first",
		"",
		...
		"   which codex",
		"   codex --version",
		"5. Restart the MultiAgent daemon:",
		...
		"",
		"When the runtime is connected, you can create MultiAgent Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 MultiAgent。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 MultiAgent 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"",
		"运行时连上后，你就可以创建 MultiAgent Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
