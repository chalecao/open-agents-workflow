...
type Config struct {
	...
	// AdminEmails is the allowlist of email addresses with platform-admin
	// privileges on this instance. Members of this set can create new
	// workspaces and perform any write against any workspace (update /
	// delete / member management / integration control) regardless of the
	// workspace-level role they hold. Users NOT in this set lose the
	// ability to do any of those operations even if they are workspace
	// owners — they can still join existing workspaces via invitation and
	// use them as normal members.
	//
	// Loaded from the MULTICA_ADMIN_EMAILS env var (comma-separated,
	// whitespace-trimmed, matched case-insensitively). When the list is
	// empty the gate is off and the previous behavior applies — every
	// caller is treated as a potential creator/manager. The matching
	// happens through isPlatformAdmin, which is the only authoritative
	// check; the UI mirrors it by reading the per-user `is_platform_admin`
	// flag from /api/me so it can hide the relevant buttons.
	AdminEmails []string
...
// isPlatformAdmin reports whether the given email belongs to the
// operator-configured AdminEmails allowlist. Empty emails never match;
// case-insensitive comparison is used so the operator can paste
// addresses in any case. The check is O(n) over the configured list,
// which is fine for the expected admin counts (single-digit operators).
//
// This is the single authoritative gate for the MULTICA_ADMIN_EMAILS
// feature — every handler that mutates workspace state (create / update
// / delete / member management / integration control) consults it. The
// UI mirrors the result via the `is_platform_admin` flag on /api/me
// so it can hide the matching buttons.
func (h *Handler) isPlatformAdmin(email string) bool {
	if email == "" || len(h.cfg.AdminEmails) == 0 {
		return false
	}
	...
	for _, candidate := range h.cfg.AdminEmails {
...
// platformAdminGateConfigured reports whether MULTICA_ADMIN_EMAILS is
// configured on this instance. When false the workspace-role-based
// authorization (owner / admin / member) applies as before; when true
// the platform-admin gate is layered on top of — and supersedes —
// the role check for the workspace write paths.
func (h *Handler) platformAdminGateConfigured() bool {
	return len(h.cfg.AdminEmails) > 0
}

// requirePlatformAdminForRequest loads the caller's user record and
// verifies they are on the operator-configured AdminEmails allowlist.
// It is intended for the workspace write paths (create / update /
// delete / member management / integration control). When the env
// var is unset the function returns ok=true without writing a
// response so the existing workspace-role middleware keeps running —
// preserving the previous behavior for self-host deployments that
// have not opted in.
//
// On rejection it writes a 403 and returns ok=false; the caller must
// return immediately. The email comes from X-User-ID (set by auth
// middleware from the JWT) followed by a single GetUser lookup, so
// each gated handler pays exactly one extra round-trip.
func (h *Handler) requirePlatformAdminForRequest(w http.ResponseWriter, r *http.Request) (db.User, bool) {
	if !h.platformAdminGateConfigured() {
		return db.User{}, true
	}
	...
	if !h.isPlatformAdmin(user.Email) {
		writeError(w, http.StatusForbidden, "platform admin privileges required")
		return db.User{}, false
	}
	return user, true
}
