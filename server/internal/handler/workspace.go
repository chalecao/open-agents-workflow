...
func (h *Handler) CreateWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	// Platform-admin gate: when MULTICA_ADMIN_EMAILS is configured,
	// only operators on that list may create new workspaces. The UI
	// hides the button via the `is_platform_admin` flag on /api/me
	// but the 403 here is the authoritative check.
	if _, ok := h.requirePlatformAdminForRequest(w, r); !ok {
		return
	}
...
func (h *Handler) UpdateWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	// Platform-admin gate: when MULTICA_ADMIN_EMAILS is configured,
	// only operators on that list may mutate workspace settings —
	// the workspace-role check below still runs as defense in depth,
	// but the platform-admin gate is the authoritative one. The UI
	// hides the matching controls via the `is_platform_admin` flag
	// on /api/me.
	if _, ok := h.requirePlatformAdminForRequest(w, r); !ok {
		return
	}
...
func (h *Handler) CreateMember(w http.ResponseWriter, r *http.Request) {
	...
	// Platform-admin gate (see CreateWorkspace for the rationale):
	// inviting / adding members is a workspace write. The UI hides
	// the "Add member" affordance for non-admins via the
	// `is_platform_admin` flag on /api/me.
	if _, ok := h.requirePlatformAdminForRequest(w, r); !ok {
		return
	}
...
func (h *Handler) UpdateMember(w http.ResponseWriter, r *http.Request) {
	...
	// Platform-admin gate (see CreateWorkspace for the rationale):
	// changing a member's role is a workspace write.
	if _, ok := h.requirePlatformAdminForRequest(w, r); !ok {
		return
	}
...
func (h *Handler) DeleteMember(w http.ResponseWriter, r *http.Request) {
	...
	// Platform-admin gate (see CreateWorkspace for the rationale):
	// removing a member is a workspace write.
	if _, ok := h.requirePlatformAdminForRequest(w, r); !ok {
		return
	}
...
func (h *Handler) DeleteWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	// Platform-admin gate (see CreateWorkspace for the rationale):
	// deleting a workspace is the most destructive workspace write
	// and is reserved for operators when the env var is set.
	if _, ok := h.requirePlatformAdminForRequest(w, r); !ok {
		return
	}
