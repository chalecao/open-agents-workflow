...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	// Handoff evaluation: when a source agent posts a comment, fire any
	// active handoff autopilots whose source_agent == author and whose
	// rules match this event. Done after triggerTasksForComment so the
	// normal comment-driven task flow (on_mention, on_comment) is
	// preserved untouched. Only runs for agent-authored comments — a
	// member posting on a source-agent handoff autopilot would create
	// a runaway loop where every member comment re-fires the handoff
	// and re-mentions the target.
	if authorType == "agent" {
		if err := h.AutopilotService.EvaluateAndDispatchHandoffAutopilots(r.Context(), service.HandoffDispatchParams{
			Issue:       issue,
			Comment:     comment,
			SourceID:    authorID,
