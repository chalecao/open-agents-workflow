...
// The bare cache lives at <root>/<workspaceID>/<repo>.git and is shared across
// all consumers in the same workspace (e.g. the local daemon and the server-
// side LLM worker). Per-task worktrees are created from the bare cache via
// `git worktree add` and torn down via `git worktree remove` + `prune`.
//
// Originally extracted from the daemon package and made module-wide so the
// server-side LLM worker (server/internal/llmexec) can reuse the same cache
// and worktree semantics instead of duplicating clone/worktree logic.
package repocache
