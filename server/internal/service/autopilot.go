...
// HandoffDispatchParams carries the inputs EvaluateAndDispatchHandoffAutopilots
// needs. Bundled in a struct so the comment hook's call site stays compact
// and so future rule kinds can be added without a signature break.
type HandoffDispatchParams struct {
	Issue     db.Issue
	Comment   db.Comment
	SourceID  string // author id of the source comment (agent UUID in practice)
	// TriggerDate is the date string that should be substituted into
	// {{date}} placeholders. Pass time.Now() at the call site so multiple
	// handoffs triggered by the same comment share a consistent stamp.
	TriggerDate string
}

// EvaluateAndDispatchHandoffAutopilots is the comment-side hook for the
// 'handoff' execution mode. The comment handler calls this once per agent
// comment, after the comment row is committed, to find every active
// handoff autopilot in the workspace whose configured source_agent matches
// the comment's author and whose rules match the event. For each match it
//
//  1. records a `running` autopilot_run (autopilot_run.source = "handoff"),
//  2. posts a comment @-mentioning the target agent with the configured
//     template body (interpolated against issue.handoff_data), and
//  3. enqueues an agent_task_queue row for the target agent using the
//     standard mention path so dedup / leader-self-trigger / private
//     gates all apply for free.
//
// Errors from individual autopilots are isolated: a failure to enqueue
// for autopilot A must not prevent autopilot B from firing. The function
// returns the first non-fatal error purely for log surface; the per-row
// state (run.status=failed, run.failure_reason) is the source of truth.
func (s *AutopilotService) EvaluateAndDispatchHandoffAutopilots(ctx context.Context, p HandoffDispatchParams) error {
	if p.SourceID == "" {
		return nil
	}
	...
	autopilots, err := s.Queries.ListActiveHandoffAutopilotsForSourceAgent(ctx, db.ListActiveHandoffAutopilotsForSourceAgentParams{
		WorkspaceID:          p.Issue.WorkspaceID,
		HandoffSourceAgentID: sourceUUID,
	})
	if err != nil {
		return fmt.Errorf("list handoff autopilots: %w", err)
	}
	if len(autopilots) == 0 {
		return nil
	}

	handoffDataMap := parseHandoffDataMap(p.Issue.HandoffData)
	eval := HandoffEvalContext{
		CommentContent:   p.Comment.Content,
		IssueHandoffData: handoffDataMap,
	}

	var firstErr error
	for i := range autopilots {
		ap := autopilots[i]
		if err := s.dispatchOneHandoff(ctx, ap, p, eval); err != nil && firstErr == nil {
			firstErr = err
		}
	}
	return firstErr
...
// dispatchOneHandoff is the per-autopilot branch. Kept as a separate
// method (not a goroutine) so the caller's context governs fan-out and
// the loop stays serial — comment volume per workspace is tiny relative
// to scheduler fan-out, and serial keeps the run_id assignment
// deterministic for audit logs.
func (s *AutopilotService) dispatchOneHandoff(ctx context.Context, ap db.Autopilot, p HandoffDispatchParams, eval HandoffEvalContext) error {
	if !ap.HandoffTargetAgentID.Valid {
		slog.Warn("handoff autopilot has no target agent, skipping",
			"autopilot_id", util.UUIDToString(ap.ID),
	...
	if !MatchesHandoffRuleSet(ruleSet, eval) {
		return nil
	}
	...
	run, err := s.Queries.CreateAutopilotRun(ctx, db.CreateAutopilotRunParams{
		...
		Status:         "running",
		TriggerPayload: nil,
		SquadID:        autopilotSquadAttribution(ap),
	})
	if err != nil {
		return fmt.Errorf("create handoff run: %w", err)
	}
	s.captureAutopilotRunStarted(ap, run, "handoff")

	// Render the comment body. The fallback order is
	//   1. handoff_comment_template (if non-empty)
	//   2. autopilot.title (so a handoff autopilot can be saved without
	//      a template and still produce a useful body)
	template := ap.HandoffCommentTemplate.String
	if template == "" {
		template = ap.Title
	}
	...
	// pipeline. This re-uses dedup (HasPendingTaskForIssueAndAgent),
	...
	// Mirror the create_issue / run_only success path: bump last_run_at
	// and publish the start event so the UI / failure monitor / WebSocket
	// fan-out all see the run as live.
	...
	// Update the run to point at the enqueued task. The mention
	// pipeline creates the task; this linkage is what makes the run
	// closeable via SyncRunFromTask when the agent finishes.
	if updated, err := s.Queries.UpdateAutopilotRunRunning(ctx, db.UpdateAutopilotRunRunningParams{
		ID:     run.ID,
		TaskID: pgtype.UUID{}, // task_id linkage is best-effort; mention pipeline doesn't expose its task id through this path
	}); err == nil {
		run = updated
	}
...
// never returns an error: a mis-shaped column is treated as "no handoff
// data", which the rule engine evaluates as a non-match for any
// issue_has_handoff_data rule. Same fall-through pattern as
// parseIssueMetadata in the handler layer.
func parseHandoffDataMap(raw []byte) map[string]any {
	if len(raw) == 0 {
