...
// can't accidentally pass a MultiAgent user UUID where a Lark open_id is
// expected.
type OpenID string

// ChatID identifies a Lark conversation (p2p or group). One ChatID maps
// to one MultiAgent chat_session via lark_chat_session_binding.
type ChatID string
...
// separate clouds with distinct hosts; a single MultiAgent deployment serves
...
const (
	...
	// DropReasonNonWorkspaceMember — the sender resolved to a MultiAgent
