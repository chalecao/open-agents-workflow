...
INSERT INTO autopilot (
    ...
    handoff_source_agent_id, handoff_target_agent_id, handoff_rules,
    handoff_rules_operator, handoff_comment_template
) VALUES (
    $1, $2, sqlc.narg('description'), $3, $4,
    $5, $6, sqlc.narg('issue_title_template'), sqlc.narg('project_id'),
    $7, $8,
    sqlc.narg('handoff_source_agent_id'), sqlc.narg('handoff_target_agent_id'),
    sqlc.narg('handoff_rules'), sqlc.narg('handoff_rules_operator'),
    sqlc.narg('handoff_comment_template')
) RETURNING *;
...
UPDATE autopilot SET
    ...
    handoff_source_agent_id = sqlc.narg('handoff_source_agent_id'),
    handoff_target_agent_id = sqlc.narg('handoff_target_agent_id'),
    handoff_rules = COALESCE(sqlc.narg('handoff_rules'), handoff_rules),
    handoff_rules_operator = COALESCE(sqlc.narg('handoff_rules_operator'), handoff_rules_operator),
    handoff_comment_template = sqlc.narg('handoff_comment_template'),
    updated_at = now()
WHERE id = $1
RETURNING *;
...
-- =====================
-- Handoff autopilot dispatch
-- =====================

-- name: ListActiveHandoffAutopilotsForSourceAgent :many
-- Looks up active handoff autopilots in a workspace whose configured source
-- agent is the given agent. Hot path for the comment-created hook: when a
-- comment is created by an agent, the handler calls this once to gather
-- every handoff autopilot that might want to fire on this comment, then
-- evaluates each autopilot's rules in-process. Partial-index-friendly —
-- the migration's idx_autopilot_handoff_source_active already covers
-- (workspace_id, handoff_source_agent_id) for status='active' rows.
SELECT * FROM autopilot
WHERE workspace_id = $1
  AND execution_mode = 'handoff'
  AND status = 'active'
  AND handoff_source_agent_id = $2;
