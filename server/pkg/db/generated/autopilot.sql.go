...
INSERT INTO autopilot (
    ...
    $14, $15,
    $16
) RETURNING id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, project_id, handoff_source_agent_id, handoff_target_agent_id, handoff_rules, handoff_rules_operator, handoff_comment_template
`
...
type CreateAutopilotParams struct {
	...
	HandoffSourceAgentID   pgtype.UUID `json:"handoff_source_agent_id"`
	HandoffTargetAgentID   pgtype.UUID `json:"handoff_target_agent_id"`
	HandoffRules           []byte      `json:"handoff_rules"`
	HandoffRulesOperator   pgtype.Text `json:"handoff_rules_operator"`
	HandoffCommentTemplate pgtype.Text `json:"handoff_comment_template"`
}
...
func (q *Queries) CreateAutopilot(ctx context.Context, arg CreateAutopilotParams) (Autopilot, error) {
	row := q.db.QueryRow(ctx, createAutopilot,
		...
		arg.ProjectID,
		arg.HandoffSourceAgentID,
		arg.HandoffTargetAgentID,
		arg.HandoffRules,
		arg.HandoffRulesOperator,
		arg.HandoffCommentTemplate,
	)
	var i Autopilot
	err := row.Scan(
		&i.ID,
		...
		&i.AssigneeType,
		&i.ProjectID,
		&i.HandoffSourceAgentID,
		&i.HandoffTargetAgentID,
		&i.HandoffRules,
		&i.HandoffRulesOperator,
		&i.HandoffCommentTemplate,
	)
	return i, err
}
...
SELECT id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, project_id, handoff_source_agent_id, handoff_target_agent_id, handoff_rules, handoff_rules_operator, handoff_comment_template FROM autopilot
WHERE id = $1
`
...
const listActiveHandoffAutopilotsForSourceAgent = `-- name: ListActiveHandoffAutopilotsForSourceAgent :many
...
type ListActiveHandoffAutopilotsForSourceAgentParams struct {
	WorkspaceID          pgtype.UUID `json:"workspace_id"`
	HandoffSourceAgentID pgtype.UUID `json:"handoff_source_agent_id"`
}
...
func (q *Queries) ListActiveHandoffAutopilotsForSourceAgent(ctx context.Context, arg ListActiveHandoffAutopilotsForSourceAgentParams) ([]Autopilot, error) {
	rows, err := q.db.Query(ctx, listActiveHandoffAutopilotsForSourceAgent, arg.WorkspaceID, arg.HandoffSourceAgentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
UPDATE autopilot SET
    ...
    project_id = $9,
    handoff_source_agent_id = $10,
    handoff_target_agent_id = $11,
    handoff_rules = COALESCE($12, handoff_rules),
    handoff_rules_operator = COALESCE($13, handoff_rules_operator),
    handoff_comment_template = $14,
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, project_id, handoff_source_agent_id, handoff_target_agent_id, handoff_rules, handoff_rules_operator, handoff_comment_template
`
