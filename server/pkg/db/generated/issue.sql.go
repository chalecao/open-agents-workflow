...
WHERE i.workspace_id = $1
  ...
  AND ($10::jsonb IS NULL OR i.handoff_data @> $10::jsonb)
  AND (
    $11::uuid IS NULL
...
type CountIssuesParams struct {
	...
	HandoffDataFilter []byte        `json:"handoff_data_filter"`
...
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, handoff_data
`
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, handoff_data FROM issue
WHERE workspace_id = $1
...
WHERE i.workspace_id = $1
  ...
  AND ($12::jsonb IS NULL OR i.handoff_data @> $12::jsonb)
  AND (
    $13::uuid IS NULL
...
WHERE i.workspace_id = $1
  ...
  AND ($8::jsonb IS NULL OR i.handoff_data @> $8::jsonb)
  AND (
    $9::uuid IS NULL
    OR (i.assignee_type = 'agent' AND i.assignee_id IN (
          SELECT a.id FROM agent a
...
const setIssueHandoffData = `-- name: SetIssueHandoffData :one
UPDATE issue SET
    handoff_data = $1::jsonb,
    updated_at = now()
WHERE id = $2 AND workspace_id = $3
RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, handoff_data
`

type SetIssueHandoffDataParams struct {
	HandoffData []byte      `json:"handoff_data"`
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) SetIssueHandoffData(ctx context.Context, arg SetIssueHandoffDataParams) (Issue, error) {
	row := q.db.QueryRow(ctx, setIssueHandoffData, arg.HandoffData, arg.ID, arg.WorkspaceID)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
