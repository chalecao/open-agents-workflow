...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each MultiAgent Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a MultiAgent user, per-installation. open_id is
...
--
-- The `multica_user_id` column name is preserved verbatim from the
-- original schema even though the brand is now "MultiAgent": renaming
-- the column would break every consumer of this table (lark dispatcher,
-- binding token service, queries generated by sqlc) and gain nothing.
-- A column is a contract; the brand in the SQL comment is just flavour.
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one MultiAgent `chat_session`. The Lark side
...
-- =====================
-- Maps a MultiAgent task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a MultiAgent user. The
