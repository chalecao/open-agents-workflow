...
description: MultiAgent は 13 個の AI コーディングツールをサポートしています。すべて同じインターフェースを実装していますが、機能の詳細は大きく異なります。
---

import { Callout } from "fumadocs-ui/components/callout";

MultiAgent は **13 個の AI コーディングツール**を標準でサポートしています。これらはすべて同じインターフェース（キューへの投入、ディスパッチ、実行、結果の返却）を実装しているため、同じ MultiAgent ボードからどれでも動かすことができます。**しかし機能の詳細は大きく異なります**: セッション再開が実際に動作するか、MCP をサポートするか、スキルファイルがどこに置かれるか、モデルをどう選択するか。このページがその完全な対応表です。
...
Qoder が提供します。CLI バイナリ名は `qodercli` です。`--yolo` を指定した非インタラクティブ print モードで動作し、ツールごとの権限プロンプトがデーモンをブロックしません。**`--model` flag はありません** — モデル選択は Qoder CLI 自体の設定（`/config` スラッシュコマンドで管理）の中にあるため、MultiAgent はこのプロバイダーでエージェントごとのモデルピッカーを無効化します。**セッション再開はベストエフォートです** — デーモンは session id が与えられたとき `-r <session-id>` を渡しますが、Qoder CLI は現在 stdout に安定した session id を返さないため、id が必要なチェインターンは `-c`/`--continue` を使う必要があります。スキルはプロジェクトの `AGENTS.md` とランタイム自身のスキルディレクトリから発見されます。MCP 連携はありません。
...
| ⚠️ コードは存在するがベストエフォート | Qoder CLI | `-r <id>` は渡されるが、CLI が安定 id を返さない; `-c`/`--continue` でチェイン |
...
**13 個のツールのうち、`mcp_config` を実際に消費するのは 7 個です: Claude Code, Codex, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw**。残りの 6 個はこのフィールドを受け取りますが、**完全に無視します** — エラーも警告もなく、構成はただ効果を発揮しません。

<Callout type="warning">
エージェント構成で `mcp_config` を設定しても、MCP 列に ✅ のないツールを選んだ場合、MCP サーバーはそのエージェントに**何の効果**も及ぼしません。MCP 連携はツール別です。
</Callout>

## スキルファイルが置かれる場所

各ツールは**それぞれ独自の**スキル探索パスを使用します。タスクが実行される前に、MultiAgent デーモンがワークスペースのスキルファイルを対応するパスにコピーします。
...
- [デーモンとランタイム](/daemon-runtimes) — ツールが実行される場所と MultiAgent への接続方法
