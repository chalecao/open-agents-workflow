...
Each runtime is backed by a specific AI coding tool. MultiAgent supports 13 of them. The most common choices:
...
The other eight (Antigravity, Hermes, Kimi, Kiro CLI, OpenCode, Pi, OpenClaw, Qoder CLI), along with each tool's full capability matrix (session resume, MCP, skill injection path, model selection), are covered in [AI coding tools comparison](/providers).
...
- [AI coding tools comparison](/providers) — full capability matrix across all 13 tools
