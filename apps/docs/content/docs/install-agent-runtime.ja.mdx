...
Qoder のエージェンティック CLI（[docs.qoder.com](https://docs.qoder.com/zh/cli/using-cli)）です。デーモンは非対話の print モードでこれを起動し、ツール呼び出し時の権限確認が stdin でブロックするのを防ぐため `--yolo` を併せて渡します。**このツールは `--model` フラグを公開していません** — モデル選択は Qoder 自身の設定で管理されるため、Multica はこのプロバイダーのエージェント単位モデルピッカーを無効化します。スキルはプロジェクトの `AGENTS.md` とランタイム自身のスキルディレクトリから自動検出されます。セッション再開は `-r <session-id>` で行います（デーモンは盲目的にチェーンする `-c` / `--continue` も受け付けます）。Qoder CLI は現在 stdout に安定したセッション id を出力しないため、明示的な id による引き継ぎは best-effort です。
...
| 備考 | `--print` はプレーンテキストをストリーム出力します。Multica はこれを行単位で `MessageText` イベントとして中継し、同じ内容を最終結果にも蓄積します。 |
