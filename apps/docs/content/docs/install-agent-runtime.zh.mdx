...
Qoder 官方的 agentic CLI（[docs.qoder.com](https://docs.qoder.com/zh/cli/using-cli)）。守护进程以非交互的 print 模式拉起它，并加 `--yolo` 跳过工具调用前的权限审批，否则第一次 shell 就会卡在等 stdin 上。**没有 `--model` flag**——模型选择保存在 Qoder 自己的设置里，所以 Multica 里这款工具的「模型」选择项也被禁用。Skill 通过项目里的 `AGENTS.md` 与运行时自带的 skill 目录自动发现。会话续接走 `-r <session-id>`（守护进程也支持 `-c`/`--continue` 做盲续）；Qoder CLI 目前不会在 stdout 上稳定打印 session id，所以显式 id 续接是 best-effort。
...
| 备注 | `--print` 输出的是纯文本，Multica 一行一行作为 `MessageText` 事件送回，并把同样的内容汇总到最终结果里。 |
