---
title: 에이전트 런타임 설치하기
description: MultiAgent는 사용자 기기에 설치된 AI 코딩 도구를 구동합니다. 이 페이지에서는 데몬이 도구를 감지할 수 있도록 지원되는 13종의 도구를 각각 설치하는 방법을 설명합니다.
---

import { Callout } from "fumadocs-ui/components/callout";

MultiAgent에서 **런타임**이란 사용자 기기의 데몬과, 데몬이 `PATH`에서 찾아낸 AI 코딩 도구 하나가 짝을 이룬 것입니다. 온보딩의 "런타임 연결" 단계에서 **지원되는 도구를 감지하지 못했습니다**라고 표시된다면, 데몬이 `PATH`를 스캔했지만 구동 방법을 아는 13종의 도구 중 어느 것도 찾지 못했다는 뜻입니다. 아래 도구 중 하나(또는 여러 개)를 설치한 다음 해당 단계로 돌아와 다시 스캔하세요. 몇 초 안에 런타임이 나타납니다.
...
<Callout type="info">
MultiAgent 서버는 사용자의 API 키나 도구 자체를 결코 보지 못합니다. 아래의 모든 것 — 설치, 인증, 모델 접근 — 은 사용자의 로컬 기기에 존재합니다. 무언가 실패한다면 거의 항상 로컬 문제입니다.
</Callout>
...
1. **MultiAgent 데몬이 실행 중이어야 합니다.** [MultiAgent CLI](/cli)를 설치한 후 `multica daemon start`를 실행하거나, 데몬을 자동으로 시작하는 [MultiAgent 데스크톱 앱](/desktop-app)을 사용하세요. 데몬이 실행 중이지 않으면 도구를 감지할 주체가 없습니다.
...
오픈 소스 CLI 에이전트 오케스트레이터입니다. MCP 구성은 MultiAgent의 작업별 config wrapper를 통해 기록됩니다. **모델은 에이전트 계층에 바인딩됩니다**(`openclaw agents add --model`) — 작업별로 재정의할 수 없으며, MultiAgent에서 `--model`이나 `--system-prompt`를 전달할 수 없습니다.
...
Google의 Antigravity CLI(`agy`)입니다. Google의 Antigravity 서비스와 짝을 이루며 Gemini 기반 모델을 실행합니다. 세션 재개는 `--conversation <id>`를 통해 작동하며, 데몬이 CLI 로그 파일에서 이를 캡처합니다. 모델 선택은 Antigravity CLI 자체 내부에서 관리됩니다 — MultiAgent는 이 제공자에 대해 에이전트별 모델 선택기를 비활성화합니다. 스킬은 `.agents/skills/`에 기록됩니다(CLI가 Gemini CLI의 워크스페이스 스킬 레이아웃을 상속함 — [Antigravity 문서](https://antigravity.google/docs/gcli-migration) 참고).
...
| 비고 | CLI는 구조화된 이벤트 스트림이 아니라 stdout에 일반 어시스턴트 텍스트를 출력합니다. 중간의 "I will run X" 줄과 최종 응답 모두 텍스트로 MultiAgent에 전달됩니다. |

### Qoder CLI (Qoder)

Qoder의 에이전틱 CLI([docs.qoder.com](https://docs.qoder.com/zh/cli/using-cli))입니다. 데몬은 비대화형 print 모드로 이를 실행하며, 도구 호출 전 권한 승인이 stdin에서 멈추는 것을 막기 위해 `--yolo`를 함께 전달합니다. **이 도구는 `--model` 플래그를 노출하지 않습니다** — 모델 선택은 Qoder 자체 설정에서 관리되므로, MultiAgent는 이 제공자에 대해 에이전트별 모델 선택기를 비활성화합니다. 스킬은 프로젝트의 `AGENTS.md`와 런타임 자체 스킬 디렉터리에서 자동 발견됩니다. 세션 재개는 `-r <session-id>`를 통해 이루어집니다(데몬은 블라인드 체이닝용 `-c`/`--continue`도 지원함). Qoder CLI는 현재 stdout에 안정적인 세션 id를 노출하지 않으므로, 명시적 id 추적은 best-effort입니다.

| | |
|---|---|
| 데몬이 찾는 이름 | `qodercli` |
| 설치 | [docs.qoder.com/zh/cli/using-cli](https://docs.qoder.com/zh/cli/using-cli)의 공식 가이드를 따르세요. CLI는 자체적으로 `PATH`에 설치됩니다. 설치 후 `which qodercli`로 확인하세요. |
| 인증 | `qodercli`를 한 번 대화형으로 실행하여 Qoder 계정 로그인을 완료하세요. |
| 비고 | `--print`은 일반 텍스트를 출력합니다. MultiAgent는 이를 줄 단위로 `MessageText` 이벤트로 전달하고 동일한 내용을 최종 결과에 누적합니다. |
...
3. **런타임 페이지를 확인하세요.** MultiAgent UI의 **런타임** 페이지에 이제 `(워크스페이스 × 도구)` 조합별로 한 행씩 나열되어야 합니다. 행에 "offline"이라고 표시되면 [데몬과 런타임 → 런타임이 오프라인으로 표시될 때](/daemon-runtimes#when-a-runtime-is-marked-offline)를 참고하세요.
