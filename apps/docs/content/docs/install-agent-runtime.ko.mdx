...
Qoder의 에이전틱 CLI([docs.qoder.com](https://docs.qoder.com/zh/cli/using-cli))입니다. 데몬은 비대화형 print 모드로 이를 실행하며, 도구 호출 전 권한 승인이 stdin에서 멈추는 것을 막기 위해 `--yolo`를 함께 전달합니다. **이 도구는 `--model` 플래그를 노출하지 않습니다** — 모델 선택은 Qoder 자체 설정에서 관리되므로, Multica는 이 제공자에 대해 에이전트별 모델 선택기를 비활성화합니다. 스킬은 프로젝트의 `AGENTS.md`와 런타임 자체 스킬 디렉터리에서 자동 발견됩니다. 세션 재개는 `-r <session-id>`를 통해 이루어집니다(데몬은 블라인드 체이닝용 `-c`/`--continue`도 지원함). Qoder CLI는 현재 stdout에 안정적인 세션 id를 노출하지 않으므로, 명시적 id 추적은 best-effort입니다.
...
| 비고 | `--print`은 일반 텍스트를 출력합니다. Multica는 이를 줄 단위로 `MessageText` 이벤트로 전달하고 동일한 내용을 최종 결과에 누적합니다. |
