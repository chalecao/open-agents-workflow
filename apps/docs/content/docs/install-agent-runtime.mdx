...
Qoder's agentic CLI ([docs.qoder.com](https://docs.qoder.com/zh/cli/using-cli)). Spawned in non-interactive print mode with `--yolo` so per-tool permission prompts never block the daemon. The CLI does **not** expose a `--model` flag — model selection is managed inside Qoder's own settings, so Multica disables the per-agent model picker for this provider. Skills are discovered from the project's `AGENTS.md` plus the runtime's own skill directories. Session resumption goes through `-r <session-id>` (the daemon also accepts `-c`/`--continue` for blind chaining); Qoder CLI does not currently surface a stable session id on stdout, so explicit id tracking is best-effort.
...
| Notes | `--print` streams plain text — Multica relays it line-by-line as `MessageText` events and accumulates the same content into the final result. |
