...
这一页带你走一遍 MultiAgent Cloud 的端到端流程——**注册 → 装 [命令行工具](/cli) → 启动 [守护进程](/daemon-runtimes) → 创建 [智能体](/agents) → 分配第一个 [任务](/tasks)**，约 5 分钟完成。
...
1. 把命令行工具配置成连接 MultiAgent Cloud
...
4. 结束后把结果发回 MultiAgent（状态变 `completed` 或 `failed`，根据是否自动重试）
