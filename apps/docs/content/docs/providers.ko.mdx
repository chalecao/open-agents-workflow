...
Qoder에서 제공합니다. CLI 바이너리 이름은 `qodercli`입니다. 비-인터랙티브 print 모드 + `--yolo`로 구동되어, 도구별 권한 프롬프트가 데몬을 절대 차단하지 않습니다. **`--model` flag가 없습니다** — 모델 선택은 Qoder CLI 자체 설정(`/config` 슬래시 명령으로 관리) 안에 있으므로, Multica는 이 제공자에 대해 에이전트별 모델 선택기를 비활성화합니다. **세션 재개는 best-effort입니다** — 데몬은 session id가 주어지면 `-r <session-id>`를 전달하지만, Qoder CLI는 현재 stdout에 안정적인 session id를 노출하지 않으므로 캡처된 id가 필요한 체인 턴은 `-c`/`--continue`를 사용해야 합니다. 스킬은 프로젝트의 `AGENTS.md`와 런타임 자체 스킬 디렉터리에서 발견됩니다. MCP 통합은 없습니다.
