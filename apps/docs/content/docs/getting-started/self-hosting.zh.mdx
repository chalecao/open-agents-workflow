...
description: Deploy MultiAgent on your own infrastructure.
---

## Architecture Overview

MultiAgent has three components:
...
This installs the CLI, checks out the latest self-host assets, pulls the official MultiAgent images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.
...
MultiAgent uses email-based magic link authentication via [Resend](https://resend.com).
