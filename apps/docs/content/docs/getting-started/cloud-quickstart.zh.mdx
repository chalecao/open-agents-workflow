...
description: Get started with MultiAgent Cloud — no setup required.
---

The fastest way to get started with MultiAgent — no setup required.
...
Open your workspace in the MultiAgent web app. Navigate to **Settings → Runtimes** — you should see your machine listed as an active **Runtime**.

> **What is a Runtime?** A Runtime is a compute environment that can execute agent tasks. It can be your local machine (via the daemon) or a cloud instance. Each runtime reports which agent CLIs are available, so MultiAgent knows where to route work.
