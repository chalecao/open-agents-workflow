...
非 Backlog 状态的 issue 一旦分配给小队，MultiAgent 会立刻给**队长智能体**入队一个 `task`（不是给每个成员都入一个）。整个流程是这样的：

1. **队长领走 task。** 队长所在的 daemon 在下次轮询时把 task 领走，和普通智能体的分配流程一样。
2. **队长拿到 briefing。** 领走的瞬间，MultiAgent 会在队长的系统提示后面追加三段内容——详见下文 [队长每次执行看到的内容](#队长每次执行看到的内容)。
