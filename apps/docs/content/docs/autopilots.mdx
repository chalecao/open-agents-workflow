...
The MultiAgent server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).
...
on the autopilot detail page; MultiAgent generates a unique URL of the shape:

```
https://<your-multica-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL — MultiAgent records a run with `source = webhook`,
...
…or any JSON object/array. MultiAgent normalizes it into an internal envelope:
...
When you don't provide an `event` field, MultiAgent infers it from common
...
comma-separated **actions** list. MultiAgent allows a webhook if **any**
...
MultiAgent derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, MultiAgent reads the
...
present, MultiAgent falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, MultiAgent
...
MultiAgent returns `200 OK` with a `status` field for normal no-op outcomes so
...
custom self-hosted reverse proxies. MultiAgent deliberately does not derive
