...
// Sidecar file that records which MultiAgent user the cached PAT in config.json
 ...
 * - Packaged: `app.getAppPath()` → `<MultiAgent.app>/Contents/Resources/app.asar`.
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "MultiAgent Desktop" }),
  });
  if (!res.ok) {
