#!/usr/bin/env bash
# MultiAgent installer — installs the CLI and optionally provisions a self-host server.
#
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/multica-ai/multica/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). MultiAgent supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing MultiAgent CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "MultiAgent CLI already installed via Homebrew"
    else
    ...
    rm -f "$brew_log"
    ok "MultiAgent CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing MultiAgent CLI from GitHub Releases..."
  ...
  rm -rf "$tmp_dir"
  ok "MultiAgent CLI installed to $bin_dir/multica"
}
...
upgrade_cli_brew() {
  info "Upgrading MultiAgent CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "MultiAgent CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "MultiAgent CLI is already the latest version"
  fi
}
...
install_cli() {
  if command_exists multica; then
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "MultiAgent CLI is up to date ($current_ver)"
      return 0
    fi

    info "MultiAgent CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    ok "MultiAgent CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. MultiAgent self-hosting requires Docker and Docker Compose.
...
# ---------------------------------------------------------------------------
setup_server() {
  info "Setting up MultiAgent server..."
  local server_ref
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning MultiAgent repository..."
  ...
  info "Pulling official MultiAgent images..."
  pull_official_selfhost_images
  info "Starting MultiAgent services (this may take a few minutes on first run)..."
  ...
  if [ "$ready" = true ]; then
    ok "MultiAgent server is running"
  else
...
run_default() {
  printf "\n"
  printf "${BOLD}  MultiAgent — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ MultiAgent CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}multica setup${RESET}                # Connect to MultiAgent Cloud (multica.ai)\n"
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  MultiAgent — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ MultiAgent server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
...
run_stop() {
  printf "\n"
  info "Stopping MultiAgent services..."
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No MultiAgent installation found at $INSTALL_DIR"
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the MultiAgent CLI"
