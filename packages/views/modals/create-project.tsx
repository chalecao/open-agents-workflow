...
export function CreateProjectModal({ onClose }: { onClose: () => void }) {
  ...
  // Per-URL default_branch_hint is kept in a separate map so we can render
  // the badge in the "Selected" list without expanding the main state shape.
  const [selectedRepos, setSelectedRepos] = useState<string[]>([]);
  const [repoHints, setRepoHints] = useState<Record<string, string>>({});
  const [repoPopoverOpen, setRepoPopoverOpen] = useState(false);
  const [repoSearch, setRepoSearch] = useState("");
  const [customRepoUrl, setCustomRepoUrl] = useState("");
  const [customRepoBranch, setCustomRepoBranch] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    if (sourceMode === "repos" && selectedRepos.length > 0) {
      resources = selectedRepos.map((url) => {
        // Only emit the hint when the user actually typed something — an
        // empty input means "use the repo's real default branch", not
        // "pin to empty string". Whitespace-only is treated the same.
        const hint = repoHints[url]?.trim();
        const ref: Record<string, unknown> = { url };
        if (hint) {
  ...
  const addCustomRepo = () => {
    ...
    if (!url) return;
    // Snapshot the branch hint at submit time. We still key the hint by URL
    // even when the URL already exists in `selectedRepos` — replacing the
    // hint for a duplicate URL is intentional (the user re-submitted the
    // form with new values), and it survives in `repoHints` so a later
    // toggle-on doesn't fall back to a stale entry.
    const hint = customRepoBranch.trim();
    if (hint) {
      setRepoHints((prev) => ({ ...prev, [url]: hint }));
    } else {
      // Clear any previously stored hint for this URL — empty input means
      // "no hint", same convention as handleSubmit.
      setRepoHints((prev) => {
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        placeholder={t(($) => $.create_project.repos_default_branch_placeholder)}
                        aria-label={t(($) => $.create_project.repos_default_branch_label)}
                        className="flex-1 bg-transparent text-xs px-1 py-1 outline-none placeholder:text-muted-foreground"
                      />
                      <Button
                        type="submit"
                        size="sm"
                    ...
                    <p className="text-[10px] text-muted-foreground leading-snug">
                      {t(($) => $.create_project.repos_default_branch_hint)}
                    </p>
                  </form>
                  {selectedRepos.length > 0 && (
