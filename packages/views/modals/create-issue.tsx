...
import type { Issue, IssueHandoffData, IssueStatus, IssuePriority, IssueAssigneeType } from "@multica/core/types";
import {
  DialogContent,
  DialogTitle,
...
import { HandoffDataDialog } from "./handoff-data-dialog";
...
export function ManualCreatePanel({
  ...
  // Handoff data is a low-frequency, advanced field — by default it lives
  // in the overflow ⋯ menu. Clicking the menu item flips the dialog open
  ...
  const handleSubmit = async () => {
    ...
    try {
      const issue = await createIssueMutation.mutateAsync({
        ...
        // Send handoff_data only when the user actually filled in a
        // payload — `{}` would be sent as "explicit empty" and the
        // server treats it the same as the column default, but omitting
        // it is cheaper and matches the rest of the optional fields.
        ...(handoffData && Object.keys(handoffData).length > 0
        ...
        {backlogHintIssueId ? (
          ...
          <>
            ...
            <div className="flex items-center gap-1.5 px-4 py-2 shrink-0 flex-wrap">
              ...
              {/* Handoff data chip — mirrors the parent/child chips above:
                  appears only when a value is set, with a click target
                  that re-opens the JSON editor. Click-X clears. Hidden
                  while the menu item is the only affordance (default). */}
              {handoffData && Object.keys(handoffData).length > 0 && (
                <div className="inline-flex items-center rounded-full border text-xs transition-colors hover:bg-accent/60">
                  <button
                    type="button"
                    onClick={() => setHandoffDataDialogOpen(true)}
                    className="flex items-center gap-1.5 py-1 pl-2.5 cursor-pointer"
                  >
                    <Braces className="size-3 text-muted-foreground" />
                    <span>
                      {t(($) => $.create_issue.handoff_data_label)}
                      <span className="text-muted-foreground">
                        {" · "}
                        {Object.keys(handoffData).length}
                      </span>
                    </span>
                  </button>
                  <button
                    ...
                    className="p-1 pr-2 text-muted-foreground hover:text-foreground cursor-pointer"
                    aria-label={t(($) => $.create_issue.handoff_data_label)}
                  >
                    <XIcon className="size-3" />
                  </button>
                </div>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-auto">
                  ...
                  {handoffData && Object.keys(handoffData).length > 0 ? (
                    <DropdownMenuItem onClick={() => setHandoffDataDialogOpen(true)}>
                      <Braces className="h-3.5 w-3.5" />
                      {t(($) => $.create_issue.edit_handoff_data)}
                    </DropdownMenuItem>
                  ) : (
                    <DropdownMenuItem onClick={() => setHandoffDataDialogOpen(true)}>
                      <Braces className="h-3.5 w-3.5" />
                      {t(($) => $.create_issue.set_handoff_data)}
                    </DropdownMenuItem>
                  )}
                  {parentIssueId && parentIssue && (
                    <>
            ...
            {/* Handoff data editor — opens inline so it stacks over this
                modal instead of replacing it via useModalStore. */}
            <HandoffDataDialog
              open={handoffDataDialogOpen}
              onOpenChange={setHandoffDataDialogOpen}
              value={handoffData}
              onSave={(next) => setHandoffData(next)}
              title={t(($) => $.create_issue.handoff_data_label)}
              description={t(($) => $.create_issue.handoff_data_hint)}
            />
