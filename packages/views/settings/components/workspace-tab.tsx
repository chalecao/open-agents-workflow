...
export function WorkspaceTab() {
  ...
  // Platform-admin gate (MULTICA_ADMIN_EMAILS): when the operator has
  // configured an allowlist, only callers on it can manage a
  // workspace — even workspace owners lose the ability to update,
  // delete, or change member roles. We layer this on top of the
  // existing role check so the UI doesn't flash a save button the
  // server is about to 403.
  ...
  const roleAllowedManage = currentMember?.role === "owner" || currentMember?.role === "admin";
  const canManageWorkspace = roleAllowedManage && !platformAdminLocked;
  const isOwner = currentMember?.role === "owner" && !platformAdminLocked;
