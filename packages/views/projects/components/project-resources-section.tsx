...
export function ProjectResourcesSection({ projectId }: { projectId: string }) {
  ...
  // Update only the default_branch_hint on a github_repo resource. We never
  // rewrite the full resource_ref here — the server normalizes a partial
  // resource_ref on PUT by merging it with the existing one, so a payload
  // that only carries { default_branch_hint } is enough to flip the bit
  // without racing on the URL field.
  const handleUpdateDefaultBranch = async (
    ...
    const previous = resource.resource_ref.default_branch_hint ?? "";
    if (trimmed === previous.trim()) return;
    try {
      await updateResource.mutateAsync({
        resourceId: resource.id,
        data: {
          resource_ref: {
            ...resource.resource_ref,
            default_branch_hint: trimmed,
          },
        },
      });
      toast.success(t(($) => $.resources.toast_default_branch_set));
    } catch (err) {
      const msg =
        err instanceof Error
          ? err.message
          : t(($) => $.resources.toast_default_branch_set_failed);
      toast.error(msg);
    }
  };
            ...
            <div className="max-h-64 space-y-1.5 overflow-y-auto pr-1">
              {resources.map((resource) => (
                <ResourceRow
                  ...
                  onUpdateDefaultBranch={handleUpdateDefaultBranch}
                />
              ))}
            </div>
          )}
  ...
  if (isGithubRef(resource)) {
    return (
      <GithubRepoRow
        resource={resource}
        onRemove={onRemove}
        onUpdateDefaultBranch={onUpdateDefaultBranch}
      />
    );
  }
...
function GithubRepoRow({
  ...
  // The hint is optional — `undefined` (server didn't send it) and `""`
  // (server stored an explicit empty) are treated identically in the UI.
  const hint = ref.default_branch_hint?.trim() ?? "";
  ...
  const commit = async () => {
    setEditing(false);
    // Server stores the trimmed form, so compare against the trimmed current
    // value to avoid a no-op round-trip that would still toast success.
    if (draft.trim() !== hint) {
      await onUpdateDefaultBranch(resource, draft);
    }
  };
  const cancel = () => {
    setEditing(false);
  ...
  return (
    <div className="flex items-center gap-2 text-xs group min-w-0">
      ...
      {editing ? (
        <input
          ...
          placeholder={t(($) => $.resources.default_branch_edit_placeholder)}
          className="w-24 shrink-0 rounded-sm border bg-transparent px-1 py-0.5 text-xs outline-none focus-visible:ring-1 focus-visible:ring-ring"
          aria-label={t(($) => $.resources.default_branch_edit_label)}
        />
      ) : hint ? (
        <Tooltip>
          <TooltipTrigger
            render={
              <span className="inline-flex shrink-0 items-center gap-0.5 rounded-sm bg-muted px-1 py-0.5 text-[10px] text-muted-foreground font-mono">
          ...
          <TooltipContent side="top">
            {t(($) => $.resources.default_branch_badge_with_branch, { branch: hint })}
          </TooltipContent>
        </Tooltip>
      ) : null}
      {!editing && (
        <button
          type="button"
          onClick={startEdit}
          className="opacity-0 group-hover:opacity-100 transition-opacity rounded-sm p-0.5 hover:bg-accent shrink-0"
          title={t(($) => $.resources.default_branch_edit_tooltip)}
        >
