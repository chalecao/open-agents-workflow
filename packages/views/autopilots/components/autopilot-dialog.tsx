...
export interface AutopilotInitial {
  ...
  // Handoff fields. The detail page passes these in edit mode so the
  // dialog can hydrate its local state; the create-mode `initial` may
  // omit them (defaults below).
  handoff_source_agent_id?: string | null;
  handoff_rules?: HandoffRule[];
  handoff_rules_operator?: HandoffRulesOperator;
  // Note: handoff_target_agent_id and handoff_comment_template are
  // intentionally absent. In handoff mode the autopilot's executor
  // (assignee) is the @-mention target, and the autopilot's description
  // doubles as the @-mention body — the dialog only needs the source
  // agent + rule set.
}
...
const OUTPUT_MODE_KEYS: AutopilotExecutionMode[] = ["create_issue", "run_only", "handoff"];
...
export function AutopilotDialog(props: AutopilotDialogProps) {
  ...
  // Handoff config. Edit mode hydrates from the server response; create
  // mode starts empty so the user has to consciously fill it in. The
  // state is only ever *read* when executionMode === "handoff", but
  // we keep it mounted across mode switches so flipping the radio back
  // and forth doesn't drop the user's in-progress edits.
  //
  // Target agent + comment template are NOT here: the autopilot's
  // executor (assignee) IS the @-mention target, and the autopilot's
  // description IS the @-mention body. The dialog only collects the
  // source agent + rule set; the rest flows from the top-level fields
  // the user is already editing.
  const [handoffSourceAgentId, setHandoffSourceAgentId] = useState<string>(
    initial.handoff_source_agent_id ?? "",
  );
  const [handoffRules, setHandoffRules] = useState<HandoffRule[]>(
    initial.handoff_rules ?? [],
  );
  const [handoffRulesOperator, setHandoffRulesOperator] = useState<HandoffRulesOperator>(
    initial.handoff_rules_operator ?? "all",
  );
  ...
  // Handoff autopilots require a source agent (whose comments drive the
  // handoff). The target is the autopilot's executor (assignee) and the
  // comment body is the autopilot's description — both are already
  // validated at the top of this form, so handoff only needs a source.
  // We surface the missing field inline in the handoff section; the
  // disabled submit button is just belt-and-braces so the user can't
  // accidentally create a misconfigured autopilot.
  const handoffConfigOk = executionMode !== "handoff" || handoffSourceAgentId.length > 0;
  const canSubmit =
    title.trim().length > 0 && assigneeId.length > 0 && handoffConfigOk && !submitting;
    ...
    try {
      if (isCreate) {
        ...
        const autopilot = await createAutopilot.mutateAsync({
          ...
          // Handoff fields. Server requires source + rules when
          // execution_mode is "handoff" — see buildHandoffRuleSet in
          // handler. The @-mention target is the autopilot's executor
          // (assignee, agent-only) and the @-mention body is the
          // autopilot's description, so we don't send those here.
          ...(isHandoff
            ? {
                handoff_source_agent_id: handoffSourceAgentId,
        ...
        // Handoff autopilots are fired by source-agent comments — they
        // don't have a user-created trigger. Skip the trigger-create
        // step entirely so we don't try to insert a "schedule" row on
        // a handoff autopilot.
        if (!isHandoff) {
          try {
        ...
        onOpenChange(false);
        // Handoff autopilots have no trigger to fail; treat the create
        // as a clean success regardless of the (skipped) triggerOk flag.
        if (isHandoff || triggerOk) {
          toast.success(t(($) => $.dialog.toast_created));
        } else {
        ...
        const wasHandoff = props.initial.execution_mode === "handoff";
        const isHandoff = executionMode === "handoff";
        await updateAutopilot.mutateAsync({
          ...
          assignee_id: assigneeId,
          execution_mode: executionMode,
          // Handoff update semantics: when the autopilot is in (or is
          // being switched to) handoff mode, send every handoff field
          // so server-side raw-presence gating doesn't drop a cleared
          // source. For non-handoff autopilots we leave them undefined
          // so the existing rows stay untouched. Target + comment
          // template are intentionally absent — they flow from the
          // autopilot's assignee + description.
          ...(isHandoff || wasHandoff
            ? {
                handoff_source_agent_id: handoffSourceAgentId || null,
                handoff_rules: handoffRules,
                handoff_rules_operator: handoffRulesOperator,
              }
            : {}),
        ...
        >
          ...
          <aside className="w-full lg:w-[340px] shrink-0 overflow-visible lg:overflow-y-auto px-5 py-5 space-y-5 bg-muted/30">
            ...
            {executionMode === "handoff" ? (
              <HandoffSection
                sourceAgentId={handoffSourceAgentId}
                rules={handoffRules}
                rulesOperator={handoffRulesOperator}
                onSourceAgentChange={setHandoffSourceAgentId}
                onRulesChange={setHandoffRules}
                onOperatorChange={setHandoffRulesOperator}
              />
            ) : (
              <>
                {isCreate && (
...
// ---------------------------------------------------------------------------
//
// Handoff autopilots do not have a user-managed trigger; they fire when
// the configured source agent posts a comment that matches the rule set.
// The handoff flow itself is server-side: on a matching event the server
// creates an autopilot_run, posts the autopilot's `description` as a
// comment @-mentioning the autopilot's executor agent, and enqueues a
// task for that agent.
//
// The dialog therefore only needs the source agent picker and the rule
// set. The target agent and comment body are the autopilot's main
// "Assignee" agent and "Runbook" (description) respectively — the
// handlers wire them together (see buildHandoffRuleSet) so the user
// never has to re-enter them.
//
// Source picker is agent-only (handoff is an agent-to-agent concept —
// squads can't be a comment author on the server side today). If a
// squad is selected as the main "assignee" above, the server rejects
// the request with a 400 ("assignee_type must be 'agent' when
// execution_mode is handoff"); the assignee picker does NOT pre-restrict
// here, so the user gets a clear validation message rather than a
// silently empty list. The handoff section just shows the source
// agent below the (squad-allowed) assignee.

const HANDOFF_RULE_KINDS: HandoffRuleKind[] = [
  "comment_contains_keyword",
  "issue_has_handoff_data",
];
...
function HandoffSection({
  ...
  rules: HandoffRule[];
  rulesOperator: HandoffRulesOperator;
  onSourceAgentChange: (id: string) => void;
  onRulesChange: (rules: HandoffRule[]) => void;
  onOperatorChange: (op: HandoffRulesOperator) => void;
}) {
  const { t } = useT("autopilots");
  const wsId = useWorkspaceId();
  const { data: agents = [] } = useQuery(agentListOptions(wsId));
  ...
  const addRule = (kind: HandoffRuleKind) => {
    onRulesChange([...rules, { kind, value: "" }]);
  };
  const updateRule = (index: number, patch: Partial<HandoffRule>) => {
    onRulesChange(
  ...
  return (
    <div className="space-y-4">
      <div>
        <SectionLabel>{t(($) => $.dialog.handoff_section.label)}</SectionLabel>
        <p className="rounded-md border bg-background px-3 py-2 text-xs text-muted-foreground leading-relaxed">
          {t(($) => $.dialog.handoff_section.hint)}
        </p>
      </div>

      <HandoffAgentPicker
        label={t(($) => $.dialog.handoff_section.source_agent)}
        agentId={sourceAgentId}
        agents={activeAgents}
        onChange={onSourceAgentChange}
        placeholder={t(($) => $.dialog.handoff_section.select_source_agent)}
        emptyHint={t(($) => $.dialog.handoff_section.source_required)}
        isEmpty={!sourceAgentId}
      />

      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <SectionLabel>{t(($) => $.dialog.handoff_section.rules_label)}</SectionLabel>
        </div>
        <p className="text-[11px] text-muted-foreground -mt-1">
          {t(($) => $.dialog.handoff_section.rules_hint)}
        </p>
        ...
        {rules.length > 0 && (
          <Select
            value={rulesOperator}
            onValueChange={(v) =>
              v && onOperatorChange(v as HandoffRulesOperator)
            }
          >
            <SelectTrigger className="w-full">
              <SelectValue />
            ...
            <SelectContent>
              <SelectItem value="all">
                {t(($) => $.dialog.handoff_section.operator_all)}
              </SelectItem>
              <SelectItem value="any">
                {t(($) => $.dialog.handoff_section.operator_any)}
              </SelectItem>
            </SelectContent>
          </Select>
        )}
            ...
            >
              <div className="flex items-center gap-1.5">
                <Select
                  value={rule.kind}
                  onValueChange={(v) =>
                    v && updateRule(i, { kind: v as HandoffRuleKind })
                  }
                >
                  ...
                  <SelectContent>
                    {HANDOFF_RULE_KINDS.map((kind) => (
                      <SelectItem key={kind} value={kind}>
                        {t(($) => $.dialog.handoff_section.rule_kind_label[kind])}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <Button
                  ...
                  title={t(($) => $.dialog.handoff_section.remove_rule)}
                >
              ...
              <input
                ...
                placeholder={
                  rule.kind === "comment_contains_keyword"
                    ? t(($) => $.dialog.handoff_section.rule_value_keyword_placeholder)
                    : t(($) => $.dialog.handoff_section.rule_value_handoff_data_placeholder)
                }
                className="w-full h-7 rounded-md border bg-background px-2 text-xs outline-none focus:ring-1 focus:ring-ring"
              />
              {rule.kind === "comment_contains_keyword" && (
                <p className="text-[10px] text-muted-foreground">
                  {t(($) => $.dialog.handoff_section.rule_value_keyword_placeholder_hint)}
                </p>
              )}
            </div>
          ))}
...
// HandoffAgentPicker is a small, agent-only picker. It's intentionally
// not the full AgentPicker (which mixes agents + squads) — the handoff
// flow is agent-to-agent today, and surfacing a squad in this list
// would just be a 400 from the server when the user submits.
function HandoffAgentPicker({
  ...
  const filtered = agents.filter((a) => matches(a.name));
  ...
  return (
    <div>
      ...
      <PropertyPicker
        ...
        trigger={
          <span className="sr-only">{t(($) => $.agent_picker.select_assignee)}</span>
        }
        triggerRender={
          <button
            type="button"
