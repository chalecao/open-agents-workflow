...
describe("WelcomeAfterOnboarding", () => {
  ...
  describe("runtime path", () => {
    it("creates a Helper agent then opens a blocking modal with starter cards", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/welcome to MultiAgent/i)).toBeInTheDocument();
      });

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.runtime_id).toBe("rt-1");
      expect(agentArgs.name).toBe("MultiAgent Helper");
      expect(agentArgs.instructions).toContain("MultiAgent Helper");

      // 3 starter card titles come from HELPER_STARTER_PROMPTS (TS const,
      // EN under the test's en locale).
      expect(
        screen.getByText("Introduce MultiAgent to me"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Walk me through the core features"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Show me what MultiAgent can do for me — as slides"),
      ).toBeInTheDocument();
    });

    it("reuses an existing MultiAgent Helper agent instead of creating duplicates", async () => {
      mockListAgents.mockResolvedValueOnce([
        {
          id: "agent-existing",
          name: "MultiAgent Helper",
    ...
    it("selecting cards then clicking Assign creates one issue per pick and navigates to the first", async () => {
      ...
      // Toggle two cards.
      fireEvent.click(screen.getByText("Introduce MultiAgent to me"));
      fireEvent.click(
        screen.getByText("Show me what MultiAgent can do for me — as slides"),
      );
      ...
      expect(titles).toEqual([
        "Introduce MultiAgent to me",
        "Show me what MultiAgent can do for me — as slides",
      ]);
    ...
    it("uses Korean persisted Helper and starter issue artifacts under ko locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("MultiAgent를 간단히 소개해 주세요"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("MultiAgent 사용 어시스턴트");
      expect(agentArgs.instructions).toContain(
        "당신은 이 MultiAgent 워크스페이스에 내장된 AI 어시스턴트",
      );

      fireEvent.click(screen.getByText("MultiAgent를 간단히 소개해 주세요"));
      fireEvent.click(
      ...
      expect(issueArgs.title).toBe("MultiAgent를 간단히 소개해 주세요");
      expect(issueArgs.description).toContain(
        "MultiAgent를 1-2문단으로 간단히 소개해 주세요",
      );
    });
    ...
    it("uses Japanese persisted Helper and starter issue artifacts under ja locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("MultiAgent を簡単に紹介してください"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("MultiAgent の使い方アシスタント");
      expect(agentArgs.instructions).toContain(
        "あなたは MultiAgent Helper、この MultiAgent ワークスペースに組み込まれた AI アシスタント",
      );

      fireEvent.click(screen.getByText("MultiAgent を簡単に紹介してください"));
      fireEvent.click(
      ...
      await waitFor(() => expect(mockCreateIssue).toHaveBeenCalledTimes(1));
      const [issueArgs] = mockCreateIssue.mock.calls[0]!;
      expect(issueArgs.title).toBe("MultiAgent を簡単に紹介してください");
      expect(issueArgs.description).toContain(
        "MultiAgent を1〜2段落で簡単に紹介してください",
      );
    });
  });
  ...
  describe("skip path", () => {
    it("provisions install-runtime → agent-guide → follow-up comment, then opens the celebration Modal", async () => {
      ...
      expect(secondCall![0].title).toBe(
        "Step 2 — Create your first MultiAgent Agent",
      );
    ...
    it("silently dismisses without showing the Modal when provisioning fails", async () => {
      ...
      expect(screen.queryByText(/Welcome to MultiAgent/i)).not.toBeInTheDocument();
    });
    ...
    it("uses Korean persisted skip-path issue and comment artifacts under ko locale", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/MultiAgent에 오신 것을 환영합니다/i)).toBeInTheDocument();
      });
      ...
      expect(installCall![0].description).toContain(
        "MultiAgent에 오신 것을 환영합니다.",
      );
      expect(guideCall![0].title).toBe(
        "2단계 — 첫 MultiAgent Agent 만들기",
      );
    ...
    it("uses Japanese persisted skip-path issue and comment artifacts under ja locale", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/MultiAgent へようこそ/),
        ).toBeInTheDocument();
      });
      ...
      expect(installCall![0].description).toContain("MultiAgent へようこそ。");
      expect(guideCall![0].title).toBe(
        "ステップ2 — 最初の MultiAgent Agent を作成する",
      );
