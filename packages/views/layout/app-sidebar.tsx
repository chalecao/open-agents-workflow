...
export function AppSidebar({ topSlot, searchSlot, headerClassName, headerStyle }: AppSidebarProps = {}) {
  ...
  // Hide "Create workspace" when either the env-level flag is on
  // (DISABLE_WORKSPACE_CREATION) or the platform-admin gate is on AND
  // the caller is not on the allowlist. We need both signals because
  // `isPlatformAdmin` alone is false in both "not on the allowlist"
  // and "no allowlist configured" — the config-level flag
  // disambiguates.
  const canCreateWorkspace = !workspaceCreationDisabled && !(adminEmailsConfigured && !isPlatformAdmin);
