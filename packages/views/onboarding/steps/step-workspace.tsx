...
export function StepWorkspace({
  ...
  // (#3433 review feedback). The platform-admin gate (MULTICA_ADMIN_EMAILS
  // configured AND caller not on the allowlist) is layered on top of the
  // self-host gate so non-admins see the same disabled flow.
  const platformAdminLocked = adminEmailsConfigured && !isPlatformAdmin;
  const workspaceCreationAllowed = !workspaceCreationDisabled && !platformAdminLocked;
      ...
      <div className="flex min-h-0 flex-col">
        ...
        >
          <div className="mx-auto w-full max-w-[620px] px-6 py-10 sm:px-10 md:px-14 lg:px-0 lg:py-14">
            <div className="mb-2 text-xs font-medium uppercase tracking-[0.08em] text-muted-foreground">
              {reusing
                ? workspaceCreationAllowed
                  ...
                  : platformAdminLocked
                    ? t(($) => $.step_workspace.admin_locked_eyebrow_resume)
                    : t(($) => $.step_workspace.creation_disabled_eyebrow_resume)
                : workspaceCreationAllowed
                  ? t(($) => $.step_workspace.eyebrow_first)
                  : platformAdminLocked
                    ? t(($) => $.step_workspace.admin_locked_eyebrow)
            ...
            <h1 className="text-balance font-serif text-[36px] font-medium leading-[1.1] tracking-tight text-foreground">
              {reusing
                ? workspaceCreationAllowed
                  ...
                  : platformAdminLocked
                    ? t(($) => $.step_workspace.admin_locked_headline_resume, { name: reusing.name })
                    : t(($) => $.step_workspace.creation_disabled_headline_resume, { name: reusing.name })
                : workspaceCreationAllowed
                  ? t(($) => $.step_workspace.headline_first)
                  : platformAdminLocked
                    ? t(($) => $.step_workspace.admin_locked_headline)
            ...
            <p className="mt-4 text-[15.5px] leading-[1.55] text-foreground/80">
              {reusing
                ? workspaceCreationAllowed
                  ...
                  : platformAdminLocked
                    ? t(($) => $.step_workspace.admin_locked_lede_resume)
                    : t(($) => $.step_workspace.creation_disabled_lede_resume)
                : workspaceCreationAllowed
                  ? t(($) => $.step_workspace.lede_first)
                  : platformAdminLocked
                    ? t(($) => $.step_workspace.admin_locked_lede)
              ...
              {reusing ? (
                <div className="flex flex-col gap-3">
                  ...
                  {/* Hide the create-new card entirely when the self-host
                      gate (DISABLE_WORKSPACE_CREATION) is on (#3433) or
                      when the platform-admin gate locks this caller out
                      (MULTICA_ADMIN_EMAILS configured, caller not on
                      the allowlist) — the backend would 403 the POST
                      and the user would be stuck with a useless form. */}
                  {workspaceCreationAllowed && (
            ...
            {!((workspaceCreationDisabled || platformAdminLocked) && !reusing) && (
