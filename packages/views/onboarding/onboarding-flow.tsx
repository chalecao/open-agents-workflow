...
export function OnboardingFlow({
  ...
  // "Locked out of creating a workspace" — same predicate as
  // StepWorkspace / NewWorkspacePage: env-level gate OR
  // platform-admin gate. Used below to short-circuit the
  // questionnaire for non-admins without an existing workspace
  // (the questionnaire is only useful once a workspace exists).
  const workspaceCreationDisabled = useConfigStore(
    (s) => s.workspaceCreationDisabled,
  );
  const adminEmailsConfigured = useConfigStore(
    (s) => s.adminEmailsConfigured,
  );
  const isPlatformAdmin = useIsPlatformAdmin();
  const platformAdminLocked = adminEmailsConfigured && !isPlatformAdmin;
  const workspaceCreationBlocked =
    workspaceCreationDisabled || platformAdminLocked;
  ...
  const handleWelcomeNext = useCallback(() => {
    ...
    //
    // Exception: when the user is locked out of workspace creation
    // (env-level gate or MULTICA_ADMIN_EMAILS not on the allowlist)
    // AND they have no existing workspace, the questionnaire is
    // pointless — its answers are persisted on the user record but
    // never read by anyone until a workspace exists, and Step 4
    // ("workspace") is the only step that has a meaningful "wait for
    // an invitation" state. Skip straight there so the user is told
    // to wait for an operator instead of being asked three more
    // questions only to land on the same dead end.
    if (workspaceCreationBlocked && !existingWorkspace) {
      setStep("workspace");
      return;
    }
    setStep(ONBOARDING_STEP_ORDER[0]!);
  }, [workspaceCreationBlocked, existingWorkspace]);
