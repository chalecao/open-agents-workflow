...
const en = `Welcome to MultiAgent.
...
const ko = `MultiAgent에 오신 것을 환영합니다.

agent가 작업을 실행하려면 먼저 runtime이 필요합니다. runtime을 설치하는 동안에도 MultiAgent를 가벼운 프로젝트 관리 워크스페이스로 먼저 사용할 수 있습니다.

## 먼저 MultiAgent를 사용해 보기
...
runtime이 연결되면 MultiAgent Helper를 만들어 안내를 받으며 첫 실행을 시작할 수 있습니다.`;

const ja = `MultiAgent へようこそ。

agent が作業を実行するには、まず runtime が必要です。runtime をインストールしている間も、MultiAgent を軽量なプロジェクト管理ワークスペースとして先に使うことができます。

## まず MultiAgent を使ってみる
...
runtime が接続されたら、MultiAgent Helper を作成して、案内付きの最初の実行を始められます。`;
