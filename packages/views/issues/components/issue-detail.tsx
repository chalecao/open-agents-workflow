...
import { HandoffDataDialog } from "../../modals/handoff-data-dialog";
...
export function IssueDetail({ issueId, onDelete, onDone, defaultSidebarOpen = true, layoutId = "multica_issue_detail_layout", highlightCommentId }: IssueDetailProps) {
  ...
  // Handoff data is editable inline via a JSON dialog. We always show
  // the trigger (key count is a useful "is anything set?" affordance);
  // only the dialog contents differ based on whether the bag has any
  // keys.
  const [handoffDataOpen, setHandoffDataOpen] = useState(false);
  ...
  // Handoff data is always an object (empty `{}` when unset), so the count
  // is just a property read on the issue body. Computing it here (after both
  // early returns) means `issue` is guaranteed non-null and the value tracks
  // every update — including optimistic mutations from `updateField`.
  const handoffKeysCount = Object.keys(issue.handoff_data ?? {}).length;
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        {propertiesOpen && <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 pl-2">
          ...
          {/* Handoff data — always rendered (any issue can carry a
              payload). The row opens a JSON editor dialog instead of a
              popover picker because the value is too large to inline.
              The displayed value is a key count or an "empty" hint,
              matching the metadata button two sections below. */}
          <PropRow label={t(($) => $.detail.prop_handoff_data)}>
            <button
              type="button"
              onClick={() => setHandoffDataOpen(true)}
              className="flex min-w-0 items-center gap-1.5 rounded-md px-1.5 py-0.5 text-xs text-muted-foreground transition-colors hover:bg-accent/60 hover:text-foreground cursor-pointer"
            >
              {handoffKeysCount > 0
                ? t(($) => $.detail.handoff_data_keys_count, { count: handoffKeysCount })
      ...
      {/* Handoff data editor — opens a JSON dialog so the user can view
          and edit the issue's handoff_data payload. Saves through
          `updateField` which already supports the tri-state PATCH
          semantics on `handoff_data` (omitted = no-op, null = clear,
          object = overwrite). */}
      <HandoffDataDialog
        open={handoffDataOpen}
        onOpenChange={setHandoffDataOpen}
        value={issue.handoff_data ?? null}
        onSave={(next) => {
          // Close the dialog first so optimistic updates don't
          // bounce the open/close animation.
          setHandoffDataOpen(false);
          handleUpdateField({ handoff_data: next });
        }}
      />
    </div>
  );
