...
/**
 * `UpdateIssueRequest` and `Partial<Issue>` are almost the same shape, but
 * `handoff_data` is tri-state in the request (omitted | null | object) while
 * the issue body never carries `null` — the server returns `{}` for cleared
 * (see IssueHandoffData docs). This helper closes the gap so optimistic
 * cache writes against `Issue`-typed queries stay type-safe.
 */
function toIssuePatch(patch: UpdateIssueRequest): Partial<Issue> {
  if (patch.handoff_data === null) {
    return { ...patch, handoff_data: {} };
  }
  // Strip `null` from the request shape (the request uses `| null` to
  // signal "clear", but `Partial<Issue>` already encodes absence as
  // `undefined`). Spreading into a new object lets TS infer the narrow
  // type without an `as` cast at every call site.
  const { handoff_data, ...rest } = patch;
  return handoff_data === undefined
    ? { ...rest }
    : { ...rest, handoff_data };
}
  ...
  return useMutation({
    ...
    onMutate: ({ id, ...data }) => {
      ...
      for (const [key, cached] of prevLists) {
        if (cached) qc.setQueryData<ListIssuesCache>(key, patchIssueInBuckets(cached, id, toIssuePatch(data)));
      }
      qc.setQueryData<Issue>(issueKeys.detail(wsId, id), (old) =>
        old ? { ...old, ...toIssuePatch(data) } : old,
      );
      if (parentId) {
        qc.setQueryData<Issue[]>(
          issueKeys.children(wsId, parentId),
          (old) =>
            old?.map((c) => (c.id === id ? { ...c, ...toIssuePatch(data) } : c)),
        );
      }
  ...
  return useMutation({
    ...
    onMutate: async ({ ids, updates }) => {
      ...
      for (const [key, cached] of prevLists) {
        if (!cached) continue;
        let next = cached;
        for (const id of ids) next = patchIssueInBuckets(next, id, toIssuePatch(updates));
      ...
      for (const [key, data] of childrenCaches) {
        ...
        qc.setQueryData<Issue[]>(issueKeys.children(wsId, parentId), (old) =>
          old?.map((c) => (idSet.has(c.id) ? { ...c, ...toIssuePatch(updates) } : c)),
        );
      }
