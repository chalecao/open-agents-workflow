...
export interface AppConfigResponse {
  ...
  // Mirrors the server's MULTICA_ADMIN_EMAILS env var (true when the
  // allowlist is non-empty). See workspace_creation_disabled above for
  // the same omit-when-false shape — keeps responses identical to the
  // pre-MULTICA_ADMIN_EMAILS contract for any deployment that hasn't
  // opted in.
...
export const AppConfigSchema = z.object({
  ...
  admin_emails_configured: BooleanWithDefaultSchema(false).optional(),
}).loose();
...
// handoff_data is a free-form JSON object (nested values are allowed —
// the rule engine and the comment-template interpolator both render the
// whole thing verbatim). Default to {} so consumers never need to
// nil-guard `issue.handoff_data`. We type it as `z.record(z.string(),
// z.unknown())` rather than `z.object({})` so the schema does not
// reject values that the wire format can legitimately emit (e.g. empty
// objects come through as `{}` which is already an object, but a future
// server may include top-level keys whose values happen to be primitives
// or arrays — we want the field to be a "JSON object" by API contract
// but still parse leniently).
const IssueHandoffDataSchema = z.record(z.string(), z.unknown()).default({});
...
export const IssueSchema = z.object({
  ...
  handoff_data: IssueHandoffDataSchema,
...
export const UserSchema = z.object({
  ...
  // Mirrors the operator-configured MULTICA_ADMIN_EMAILS allowlist on
  // the backend. When true, the caller may create new workspaces and
  // perform every write against every workspace regardless of the
  // workspace-level role they hold. The UI uses it to hide the
  // matching "Create workspace" / "Settings" affordances for
  // non-admins. Backwards-compatible default is `false` for older
