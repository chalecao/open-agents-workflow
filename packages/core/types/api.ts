import type { Issue, IssueHandoffData, IssueMetadata, IssueStatus, IssuePriority, IssueAssigneeType } from "./issue";
...
export interface CreateIssueRequest {
  ...
  /** Optional JSON payload consumed by the autopilot handoff engine. Pass
   *  any JSON object — the server enforces object shape + an 8 KiB cap
   *  (see server handler). When omitted, the column default '{}' applies. */
  handoff_data?: IssueHandoffData | null;
}
...
export interface UpdateIssueRequest {
  ...
  attachment_ids?: string[];
  /** Tri-state PATCH: omitted → leave column alone, explicit {} → clear it.
   *  Same shape constraints as CreateIssueRequest.handoff_data. */
  handoff_data?: IssueHandoffData | null;
}
