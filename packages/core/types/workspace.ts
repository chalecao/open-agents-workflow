...
export interface User {
   ...
   * non-admins. See the matching `isPlatformAdmin` field on the User
   * schema (`packages/core/api/schemas.ts`) and the backend
   * `isPlatformAdmin` helper in `server/internal/handler/handler.go`
