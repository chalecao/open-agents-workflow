...
// `handoff` was added in the handoff-pilot feature. The "agent source
// posted a comment" event drives a handoff autopilot, which then
// @-mentions a target agent with a templated body. Older servers do
// not return this value, so the UI must tolerate unknown values
// (treat as `create_issue` for fallback behavior).
export type AutopilotExecutionMode = "create_issue" | "run_only" | "handoff";
...
// Handoff rule kinds. Mirrors the server enum in service.HandoffRuleKind —
// keep both lists in lock-step when adding a new rule kind.
export type HandoffRuleKind =
  | "comment_contains_keyword"
  | "issue_has_handoff_data";

// `all` = every rule must match; `any` = at least one rule must match.
// Mirrors service.HandoffRulesOperator.
export type HandoffRulesOperator = "all" | "any";

export interface HandoffRule {
  kind: HandoffRuleKind;
  // For comment_contains_keyword, the substring (case-insensitive).
  // For issue_has_handoff_data, the top-level handoff_data key to look for.
  value: string;
}
...
export interface Autopilot {
  ...
  // Handoff-only fields. The server omits them from non-handoff
  // autopilots (handler uses pointer + omitempty), so the consumer
  // should switch on `execution_mode === "handoff"` before reading
  // any of these.
  ...
  handoff_comment_template?: string | null;
...
export interface CreateAutopilotRequest {
  ...
  // Handoff fields. Required when execution_mode === "handoff"; the
  // server returns a 400 if any are missing or out-of-workspace.
  handoff_source_agent_id?: string;
  handoff_target_agent_id?: string;
  handoff_rules?: HandoffRule[];
  handoff_rules_operator?: HandoffRulesOperator;
  handoff_comment_template?: string;
}
...
export interface UpdateAutopilotRequest {
  ...
  execution_mode?: AutopilotExecutionMode;
  issue_title_template?: string | null;
  // Handoff fields. Each is gated on raw-presence by the server so
  // "field omitted" means "no change" and "field present" means
  // "overwrite". The handoff_* fields are only meaningful when the
  // autopilot is in (or is being switched to) execution_mode=handoff.
  handoff_source_agent_id?: string | null;
  handoff_target_agent_id?: string | null;
  handoff_rules?: HandoffRule[];
  handoff_rules_operator?: HandoffRulesOperator;
