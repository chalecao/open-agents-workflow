...
- **稳定路由** —— 直接 `@FrontendTeam` 而不是 `@alice-or-bob`,团队成员变更不影响任务分配
...
multica squad member add <squad-id> --member-id <agent-uuid> --type agent --role "Owns Tailwind / shadcn"
multica squad member add <squad-id> --member-id <agent-uuid> --type agent --role "Reviewer of last resort"
```
...
# 2. 创建一个 agent(在 Web 端:Settings → Agents → New Agent)

# 3. 创建 issue 并分配给 agent,看它自动执行
multica issue create --title "Fix login bug" --assignee my-agent
```
...
贡献者请参阅 [Contributing Guide](CONTRIBUTING.md)。
